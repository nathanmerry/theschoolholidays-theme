...
<body class="w-full h-screen bg-base text-base-content">
  <div class="">
    ...
    <div class="overflow-hidden bg-center bg-cover bg-base-200" style="background-image: url(http://localhost/wp-content/uploads/2023/08/hero.jpg)">
      <div class="container pt-20 mx-auto pb-28">
    ...
    <form id="mainForm" style="transform: translateY(-55px); margin-bottom: -55px" class="container flex items-end gap-8 px-4 pt-2 pb-4 mx-auto bg-[#2b6fb1]">
      ...
      <div class="w-1/3">
        ...
        <input class="w-full input input-bordered" type="number" min="0" name="distance" placeholder="Distance (KM)" value="<?php echo $_GET['distance'] ?? null ?>">
      </div>

      <button class="w-1/3 btn outline-none border-0 text-[#203240] bg-[#fdb900]">Search</button>
    </form>
    <section class="py-10 text-left">
      <div class="container mx-auto">
        <?php if (count($camps->campData)) : ?>
          <div class="flex grid grid-cols-3 gap-4 gap-5">
            <?php foreach ($camps->campData as $camp) : ?>
              <div class="flex flex-col justify-between w-full overflow-hidden rounded shadow-xl card bg-base-100">
                <div class="flex-1">
                  <div class="relative">
                    <div class="h-[220px] flex items-center bg-gray-100">
                ...
                <div class="w-full px-5 pb-4">
                  <a href="<?php echo $camp['url'] ?>" class="w-full no-underline btn bg-[#2b6fb1] text-white">Visit Website <i class="fa-solid fa-arrow-right"></i></a>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
